...
function menu_collapse (n_id) {
	...
	for (n_id = 0; n_id < this.a_index.length; n_id++) {
		...
		if (o_curritem.n_depth > n_tolevel && o_curritem.b_visible) {
			...
			showDiv(o_curritem.e_oelement, false);
...
function menu_expand (n_id) {
	...
	for (var n_order = 0; n_order < o_item.a_children.length; n_order++) {
		...
		qq_min = qq;
		showDiv(o_curritem.e_oelement, true);
		qq_max = qq;
...
var qq=0,qq_min=0,qq_max=0;

function showDiv(sObj, bState) {
    var oDiv = sObj;
    qq++;
    ...
    if(bState){
		...
		if( isIEVersion(6) ){
		    try {
		        var oIframe = document.getElementById('HelpFrame_' + oDiv.id);
		        oIframe.scrolling = 'no';
		    }
		    catch (e) {
		        var oIframe = document.createElement('iframe');
		        var oParent = oDiv.parentNode;
		        oIframe.id = 'HelpFrame_' + oDiv.id;
		        oIframe.name = 'HelpFrame_' + oDiv.id;
		...
		//relocate menu position
		menu_left = elementLeft(oDiv);
		menu_width = oDiv.clientWidth;
		avail_width = document.documentElement.clientWidth;
		//alert(menu_left + "," + menu_width + "," + avail_width);
		//alert( parseInt(menu_left + menu_width) );
		menu_pos = avail_width - menu_width + 'px';
		if(menu_left + menu_width > avail_width) oDiv.style.left = menu_pos;
		
    } else {
        if(isIEVersion(6)) // IE 6
        	document.getElementById("HelpFrame_" + oDiv.id).removeNode(false);
...
function isIEVersion(version){
	return (isIE() && ie_version() == version);
}
...
function elementLeft(tmpDiv){

	var left = 0;
	while(tmpDiv != null){
		left += tmpDiv.offsetLeft;
		tmpDiv = tmpDiv.offsetParent;
	}
